...
/**
 * Provides a watchdog that periodically emits a heartbeat event that tells you if a
 * service is still around.
 */
@Configuration
...
public class JobWatchdogAutoConfiguration {
	...
	Watchdog watchdog(WatchdogProperties properties, GenericApplicationContext applicationContext) {
		return new Watchdog(this.scheduledExecutorService(), applicationContext,
				properties.inactivityHeartbeatInSeconds(), properties.inactivityThresholdInSeconds());
	}
...
record WatchdogProperties(long inactivityThresholdInSeconds, long inactivityHeartbeatInSeconds) {
}
...
class Watchdog implements InitializingBean {
	...
	Watchdog(Executor executor, GenericApplicationContext applicationContext, long inactivityHeartbeatInSeconds,
	...
	private void logMemory() {
		if (this.log.isDebugEnabled()) {
			var msg = Map.of("free memory", Runtime.getRuntime().freeMemory(), "max memory",
					Runtime.getRuntime().maxMemory(), "total memory", Runtime.getRuntime().totalMemory());
			...
			this.log.debug(Instant.now().atZone(ZoneId.systemDefault()));
		}
	}
